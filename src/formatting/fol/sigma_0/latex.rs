...
impl Display for Format<'_, Relation> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self.0 {
            ...
            Relation::GreaterEqual => write!(f, "\\geq"),
            Relation::LessEqual => write!(f, "\\leq"),
...
impl Display for Format<'_, BinaryConnective> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self.0 {
            BinaryConnective::Equivalence => write!(f, "\\lrar"),
            BinaryConnective::Implication => write!(f, "\\rar"),
            BinaryConnective::ReverseImplication => write!(f, "\\leftarrow"),
            BinaryConnective::Conjunction => write!(f, "\\wedge"),
            BinaryConnective::Disjunction => write!(f, "\\vee"),
        }
    }
}
